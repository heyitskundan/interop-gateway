...
```bash
npm install @interop-gateway/mcp
```

## Run as a standalone MCP server (stdio)

```bash
npx @interop-gateway/mcp
```
...
```bash
claude mcp add interop-gateway -- node /absolute/path/to/interop-gateway/packages/mcp/dist/cli.js
```
...
- **`validateUsCore`** — `{ payload: string }` (a FHIR resource or Bundle as a JSON
  ...
  resource) or `UsCoreValidationResult[]` (Bundle) from `@interop-gateway/core`
...
- **`connect_ehr`** — `{ baseUrl: string, auth: AuthConfig, scopes: GrantedScope[] }` →
  ...
  underlying `@interop-gateway/connector` package supports both auth methods;
