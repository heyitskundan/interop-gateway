...
export interface CreateInteropGatewayMcpServerOptions extends PersistenceOptions {
   ...
   * `persistence.audit.directory` (or a `./mcp-server-audit` default under `baseDir`)
 ...
 * `connector-smart-generic` package supports both; this server exposes them as
...
export async function createInteropGatewayMcpServer(
  ...
  server.registerTool(
    ...
    async ({ baseUrl, auth, scopes }) => {
      const { correlationId } = createEnvelope(baseUrl, "mcp-server:connect_ehr");
      try {
  ...
  server.registerTool(
    ...
    async ({ authorizeUrl, tokenUrl, clientId, clientSecret, redirectUri, scope, aud, launch }) => {
      const { correlationId } = createEnvelope(authorizeUrl, "mcp-server:start_smart_launch");
      try {
  ...
  server.registerTool(
    ...
    async ({ state, code, baseUrl, scopes }) => {
      const { correlationId } = createEnvelope(state, "mcp-server:complete_smart_launch");
  ...
  server.registerTool(
    ...
    async ({ connectionId, resourceType, id, searchParams }) => {
      const { correlationId } = createEnvelope(resourceType, "mcp-server:read_resource");
  ...
  server.registerTool(
    ...
    async ({ connectionId, operation, resourceType, id, resource }) => {
      const { correlationId } = createEnvelope(resourceType, "mcp-server:write_resource");
      const client = connections.get(connectionId);
      if (!client) {
  ...
  server.registerTool(
    ...
    async ({ connectionId, level, groupId, types, since, typeFilter, outputFormat }) => {
      const { correlationId } = createEnvelope(connectionId, "mcp-server:start_bulk_export");
      const client = connections.get(connectionId);
      if (!client) {
  ...
  server.registerTool(
    ...
    async ({ connectionId, exportId }) => {
      const { correlationId } = createEnvelope(exportId, "mcp-server:check_bulk_export_status");
      const client = connections.get(connectionId);
  ...
  server.registerTool(
    ...
    async ({ connectionId, type, url, requiresAccessToken }) => {
      const { correlationId } = createEnvelope(url, "mcp-server:download_bulk_export_file");
      const client = connections.get(connectionId);
      if (!client) {
  ...
  server.registerTool(
    ...
    async ({ connectionId, exportId }) => {
      const { correlationId } = createEnvelope(exportId, "mcp-server:cancel_bulk_export");
      const client = connections.get(connectionId);
      const entry = bulkExports.get(exportId);
      if (!client || !entry || entry.connectionId !== connectionId) {
  ...
  server.registerTool(
    ...
    async ({ host, port, message, timeoutMs, maxAttempts }) => {
      const { correlationId } = createEnvelope(message, "mcp-server:send_message");
      try {
  ...
  server.registerTool(
    ...
    async ({ yamlConfig }) => {
      const { correlationId } = createEnvelope("run_pipeline", "mcp-server:run_pipeline");
      try {
  ...
  server.registerTool(
    ...
    async ({ pipelineId }) => {
      const { correlationId } = createEnvelope(pipelineId, "mcp-server:stop_pipeline");
