  ...
  return (
    <div>
      ...
      <p style={muted}>
        ...
        <code>@interop-gateway/connector</code>'s <code>SmartClient</code> (<code>read()</code>/
      ...
      <p style={muted}>
        ...
        connectivity half (<code>@interop-gateway/connector</code>) is a vendor-agnostic SMART on
        FHIR client; <code>@interop-gateway/protocol</code> adds MLLP/HTTP/file-drop adapters that
        don't care which format is travelling over them. See{" "}
        <a
          href="#"
          onClick={(e) => {
            e.preventDefault();
        ...
        and the other package tabs in the sidebar for what each one actually does.
      </p>
      ...
      <p style={muted}>
        Install only the package(s) you need — installing <code>@interop-gateway/core</code> alone
        never pulls in the MCP SDK, Vault client, or AWS SDK.
      </p>
...
result.value; // the FHIR Bundle`,
      ...
      />
      <p style={muted}>
        Swap in <code>formatCda</code> (same package, <code>@interop-gateway/core</code>) and{" "}
        ...
        <tbody>
          <tr>
            ...
            <td className="text-muted">
              <code>@interop-gateway/core</code>'s translation also runs in a browser bundle
            </td>
          </tr>
          <tr>
            <td>TypeScript</td>
          ...
          <tr>
            ...
            <td>Only if used</td>
            <td className="text-muted">
              <code>@interop-gateway/secrets</code>'s Vault/AWS providers are opt-in — the dev
              default is its keychain provider (OS keychain, no external service)
            </td>
          </tr>
        </tbody>
      </table>
